...
import {
  collection, doc, setDoc, getDocs, deleteDoc, updateDoc, query, where, orderBy
...
const css = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── AUTH ── */
  .auth-screen { min-height: 100dvh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 24px; background: ${G.bg}; }
  .auth-card { background: ${G.bgCard}; border: 1px solid ${G.border}; border-radius: 16px; padding: 40px 32px; max-width: 400px; width: 100%; text-align: center; }
  .auth-logo { font-family: 'Cormorant Garamond', serif; font-size: 2.8rem; font-weight: 700; color: ${G.gold}; margin-bottom: 8px; }
  .auth-subtitle { font-size: 0.9rem; color: ${G.textMuted}; margin-bottom: 32px; }
  .auth-desc { font-size: 0.8rem; color: ${G.textDim}; margin-top: 16px; line-height: 1.6; }
  .btn-google { display: flex; align-items: center; justify-content: center; gap: 12px; width: 100%; padding: 12px 20px; border-radius: 8px; border: 1px solid ${G.borderStrong}; background: rgba(255,255,255,0.05); color: ${G.text}; font-size: 0.9rem; font-weight: 500; cursor: pointer; transition: all 0.2s; font-family: 'Outfit', sans-serif; }
  .btn-google:hover { background: rgba(255,255,255,0.1); transform: translateY(-1px); }
  .btn-google svg { width: 20px; height: 20px; flex-shrink: 0; }
  .user-avatar { width: 32px; height: 32px; border-radius: 50%; border: 1px solid ${G.borderStrong}; cursor: pointer; }
  .user-menu { position: relative; display: flex; align-items: center; gap: 8px; }
  .user-name { font-size: 0.78rem; color: ${G.textMuted}; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .sync-badge { font-size: 0.65rem; padding: 2px 7px; border-radius: 10px; background: rgba(74,124,89,0.15); color: #7fcf92; border: 1px solid rgba(74,124,89,0.3); white-space: nowrap; }
  .sync-badge.syncing { background: rgba(201,168,76,0.1); color: ${G.gold}; border-color: rgba(201,168,76,0.25); }
...
export default function App() {
  ...
  async function saveToFirestore(key, data) {
    if (!user) return;
    try {
      await setDoc(doc(db, 'users', user.uid, 'drafts', key), data);
    } catch(e) { console.error('Firestore save error', e); }
  }

  async function deleteFromFirestore(key) {
    if (!user) return;
    try {
      await deleteDoc(doc(db, 'users', user.uid, 'drafts', key));
  ...
  async function loadFromFirestore() {
    ...
    setSyncing(true);
    try {
      const snap = await getDocs(collection(db, 'users', user.uid, 'drafts'));
      const data = snap.docs.map(d => d.data()).sort((a,b) => b._ts - a._ts);
      setDrafts(data);
      // Sync companies
      const compSnap = await getDocs(collection(db, 'users', user.uid, 'companies'));
      if (compSnap.docs.length > 0) {
        const comps = compSnap.docs.map(d => d.data());
        setCompanies(comps);
        lsSet('md_companies', comps);
      }
  ...
  async function saveCompanyToFirestore(company) {
    if (!user) return;
    try {
      await setDoc(doc(db, 'users', user.uid, 'companies', String(company.id)), company);
    } catch(e) { console.error(e); }
  }

  async function deleteCompanyFromFirestore(id) {
    if (!user) return;
    try {
      await deleteDoc(doc(db, 'users', user.uid, 'companies', String(id)));
    } catch(e) { console.error(e); }
  }

  // Charger depuis Firestore quand l'utilisateur se connecte
  useEffect(() => {
  ...
  function saveCompanies(list) {
    setCompanies(list);
    lsSet('md_companies', list);
    list.forEach(c => saveCompanyToFirestore(c));
  }
  ...
  const renderAuth = () => (
    <div className="auth-screen">
      <div className="auth-card">
        <div className="auth-logo">Mydevis</div>
        <div className="auth-subtitle">Gestion de devis professionnels</div>
        ...
        <div className="auth-desc">
          Connectez-vous pour synchroniser vos devis sur tous vos appareils (téléphone, PC, tablette).
        </div>
      </div>
    </div>
  );
  ...
  const renderHome = () => (
    <div className="home-page">
      {/* User bar */}
      {user && (
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,padding:'10px 14px',background:G.bgCard,border:`1px solid ${G.border}`,borderRadius:10}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            {user.photoURL && <img src={user.photoURL} className="user-avatar" alt="avatar" />}
            <div>
              <div style={{fontSize:'0.82rem',color:G.text,fontWeight:500}}>{user.displayName}</div>
              <div style={{fontSize:'0.7rem',color:G.textMuted}}>{user.email}</div>
            </div>
          </div>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <span className={`sync-badge ${syncing?'syncing':''}`}>{syncing?'⟳ Sync...':'☁ Synchronisé'}</span>
  ...
  if (authLoading) return (
    <>
      <style>{css}</style>
      <div style={{minHeight:'100dvh',display:'flex',alignItems:'center',justifyContent:'center',background:G.bg}}>
        <div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:'2rem',color:G.gold}}>Mydevis…</div>
      </div>
    </>
  );
